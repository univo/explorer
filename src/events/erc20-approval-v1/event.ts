...
export interface Erc20ApprovalV1 {
...
// CREATE TABLE event_erc20_approval_v1 (
//     `id` FixedString(36),
//     `success` Bool,
//     `quantity` UInt256,
			...
			.map((log) => {
				try {
					...
					const id = createId({
						...
						table_id: tables.erc20_approval_v1,
...
univo.event({
	...
	id: "erc20_approval_v1_index_account_v1",
	handler: (block) => {
...
univo.event({
	...
	id: "erc20_approval_v1_index_tx_hash_v1",
	handler: (block) => {
		return event.handler(block).flatMap((event) => [
...
univo.event({
	...
	id: "erc20_approval_v1_index_block_number_v2",
	handler: (block) => {
		return event.handler(block).flatMap((event) => [
...
export async function getErc20ApprovalV1(ids: string[]) {
	const filtered = ids.filter((id) => parseId(id).table_id === tables.erc20_approval_v1);
	...
	const res = await db.query({
		query: `SELECT id, success, toString(quantity) as quantity, owner_address, spender_address, token_address FROM event_erc20_approval_v1 WHERE id IN (${mapped.join(",")});`,
		format: "JSONEachRow",
	});

	const rows: any[] = await res.json();

	const events = rows.map<Erc20ApprovalV1>((row) => {
		return {
			tag: "erc20_approval_v1",
			id: row.id as string,
