...
export interface LogErc721TransferV2 {
			...
			return receipt.logs.flatMap<LogErc721TransferV2>((log) => {
				try {
					...
					const id = createId({
						...
						tableId: TABLES.log_erc721_transfer_v2,
...
export async function getLogErc721TransferV2(ids: string[]) {
	const mapped = ids.map((id) => parseId(id));

	const filtered = mapped.filter((id) => id.tableId === TABLES.log_erc721_transfer_v2);
	...
	return rows.map<LogErc721TransferV2>((row) => {
		const id = createId({
