...
import { event, getErc20TransferV3 } from "./event";
...
test.concurrent("erc20_transfer_v3", async ({ expect }) => {
	...
	const initial = await getErc20TransferV3(ids);

	expect(initial).toStrictEqual([]);

	await test_client.request({
		method: "private_writeEvents",
		params: [{ events: ["erc20_transfer_v3"], blocks: [block] }],
	});

	const events = await getErc20TransferV3(ids);
