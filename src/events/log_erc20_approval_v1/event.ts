...
export interface LogErc20ApprovalV1 {
...
univo.event({
	...
	id: "log_erc20_approval_v1_index_block_number_tx_index_v4",
	handler: (block) => event.handler(block).map((event) => event.id),
});

export async function getLogErc20ApprovalV1(ids: string[]) {
	const filtered = ids.filter((id) => parseId(id).tableId === TABLES.log_erc20_approval_v1);
	...
	return rows.map<LogErc20ApprovalV1>((result) => {
		return {
			tag: "log_erc20_approval_v1",
			id: result.id,
