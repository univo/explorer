			...
			return receipt.logs.flatMap<LogErc721TransferV1>((log) => {
				try {
					...
					const id = createId({
						...
						tableId: TABLES.log_erc721_transfer_v1,
...
univo.event({
	...
	id: "log_erc721_transfer_v1_index_block_number_tx_index_v4",
	handler: (block) => event.handler(block).map((event) => event.id),
});

export async function getLogErc721TransferV1(ids: string[]) {
	const filtered = ids.filter((id) => parseId(id).tableId === TABLES.log_erc721_transfer_v1);
	...
	return rows.map<LogErc721TransferV1>((result) => {
		return {
			tag: "log_erc721_transfer_v1",
			id: result.id,
