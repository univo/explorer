	...
	storage: {
		async upsert(batch) {
			...
			for (let i = 0; i < batch.length; i += MAX_BATCH_SIZE) {
				await client
					.insert(schema.event_erc20_approval_v3)
					.values(batch.slice(i, i + MAX_BATCH_SIZE))
					.onConflictDoUpdate({
						target: schema.event_erc20_approval_v3.id,
						set: {
							success: sql.raw(`excluded.${schema.event_erc20_approval_v3.success.name}`),
							quantity: sql.raw(`excluded.${schema.event_erc20_approval_v3.quantity.name}`),
							owner_address: sql.raw(`excluded.${schema.event_erc20_approval_v3.owner_address.name}`),
							spender_address: sql.raw(`excluded.${schema.event_erc20_approval_v3.spender_address.name}`),
							token_address: sql.raw(`excluded.${schema.event_erc20_approval_v3.token_address.name}`),
						},
					});
			}
		},
		...
		async delete(batch) {
			...
			await client.delete(schema.event_erc20_approval_v3).where(
				inArray(
					schema.event_erc20_approval_v3.id,
...
export async function getErc20ApprovalV3(ids: string[]) {
	...
	const results = await client
		.select()
		.from(schema.event_erc20_approval_v3)
		.where(inArray(schema.event_erc20_approval_v3.id, filtered))
		.orderBy(asc(schema.event_erc20_approval_v3.id));

	return results.map<Erc20ApprovalV3>((result) => {
		return {
