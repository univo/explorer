...
import { event, getIntentNativeTransferV1 } from "./event";
...
test.concurrent("intent_native_transfer_v1", async ({ expect }) => {
	...
	const initial = await getIntentNativeTransferV1(ids);
		...
		params: [
			{
				blocks: [block],
				events: [
					"intent_native_transfer_v1",
					"intent_native_transfer_v1_index_account_v3",
					"intent_native_transfer_v1_index_block_number_tx_index_v4",
				],
			},
		],
	});

	const events = await getIntentNativeTransferV1(ids);
