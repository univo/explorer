...
import { event, getIntentIdmV1 } from "./event";
import { test_getBlock, test_client } from "@/tests/utils";

test.concurrent("intent_idm_v1", async ({ expect }) => {
	...
	const initial = await getIntentIdmV1(ids);
		...
		params: [
			{
				...
				events: [
					"intent_idm_v1", //
					"intent_idm_v1_index_account_v3",
					"intent_idm_v1_index_block_number_tx_index_v4",
				],
			},
		],
	});

	const events = await getIntentIdmV1(ids);

	expect(events).toMatchInlineSnapshot(`
		[
		  {
		    "from_address": "0x878761636a1Dd513463B04A66413C77E8B4eEDEd",
		    "id": "68dd2e57016653980015ffffff00010022",
